...
interface AnnouncementDocumentFormProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (data: FormData) => void;
  initialData?: AnnouncementDocument | null;
  isLoading?: boolean;
}

const urgencyOptions: UrgencyLevel[] = ['ด่วนที่สุด', 'ด่วนมาก', 'ด่วน', 'ปกติ'];

export function AnnouncementDocumentForm({
  open,
  onOpenChange,
  onSubmit,
  initialData,
  isLoading,
}: AnnouncementDocumentFormProps) {
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">
            {initialData ? "แก้ไขหนังสือประกาศ" : "เพิ่มหนังสือประกาศ"}
          </DialogTitle>
        </DialogHeader>
        <Form {...form}>
